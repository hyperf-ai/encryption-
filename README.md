...
```shell script
composer require hyperf-ai/encryption
```
...
你可以使用 `\HyperfAi\Encryption\Crypt` 类来加密一个值。所有加密过的值都会使用消息认证码 (MAC) 来签名，以检测加密字符串是否被篡改过：
...
实现 `\HyperfAi\Hashing\Encryption\SymmetricDriverInterface` 或 `\HyperfAi\Hashing\Encryption\AsymmetricDriverInterface` 接口，并参照配置文件中的其他算法进行配置。
